...
$ticketList = $tickets instanceof \Illuminate\Support\Collection ? $tickets->all() : (is_array($tickets) ? $tickets : []);
    ...
    <div class="section-container relative z-10">
        ...
        <div class="max-w-lg mx-auto">
            ...
            <div class="space-y-3 mb-6">
                ...
                @php $ticketCode = is_array($ticket) ? ($ticket['ticket_code'] ?? null) : ($ticket->ticket_code ?? null); $ticketName = is_array($ticket) ? ($ticket['participant'] ?? 'Peserta') : ($ticket->participant ?? 'Peserta'); @endphp
                @if($ticketCode)
                <a href="{{ route('ticket.show', $ticketCode) }}" class="card p-4 flex items-center gap-4 hover:border-[#2563EB] transition-colors group">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="font-bold text-[#0B1040] text-sm truncate">{{ $ticketName }}</p>
                        <p class="font-mono text-xs text-[#2563EB]">{{ $ticketCode }}</p>
                    </div>
            ...
            <div class="space-y-3 mb-6">
                <p class="text-xs font-bold text-[#94A3B8] uppercase tracking-widest">E-Ticket Kamu</p>
                <div class="text-sm text-[#64748B]">Belum ada e-ticket yang dibuat.</div>
            </div>
            @endif

            <div class="flex flex-col sm:flex-row gap-3">
                @if($ticketCount > 0)
                    <a href="{{ route('ticket.group', $order_code) }}" class="btn-primary btn-lg flex-1 justify-center">
                        Lihat Semua E-Ticket
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
                        </svg>
                    </a>
                @else
                    <span class="btn-primary btn-lg flex-1 justify-center opacity-60 cursor-not-allowed">Lihat E-Ticket</span>
                @endif
